...
namespace StackExchange.Exceptional.Stores
{
    ...
    public sealed class JSONErrorStore : ErrorStore
    {
        ...
        {
            Size = Math.Min(settings.Size, MaximumSize);
            Path = settings.Path.ResolvePath();
        }
        ...
        protected override bool DeleteAllErrors(string applicationName = null)
        {
            ...
            foreach (var fn in fileList)
            {
                ...
                {
                    Debug.WriteLine("Error Deleting file '" + fn + "' from the store: " + ex.Message);
                }
            }
            return deleted > 0;
        }
        ...
        protected override void LogError(Error error)
        {
            ...
            else
            {
                ...
                string fileName = $@"{Path}\error-{timeStamp}-{detailHash}-{error.GUID.ToFileName()}.json";
        ...
        {
            string[] fileList = Directory.GetFiles(Path, $"*{guid.ToFileName()}.json");
        ...
        {
            string[] files = Directory.GetFiles(Path, "error*.*");

            if (files.Length <= Size) return; // room left below the cap
            
            Array.Sort(files); // sort by timestamps

            // we'll remove any errors with index less than this upper bound
            int upperBound = files.Length - Size;
